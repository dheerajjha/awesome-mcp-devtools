...
- [dheerajjha/mcp-migrate](https://github.com/dheerajjha/mcp-migrate) 🐍 - Finds what the 2026-07-28 spec revision breaks in an MCP server and mechanically fixes what can be fixed. 21 rules covering removed sessions, `initialize`, `ping` and `resources/subscribe`, 5 autofixers that rewrite source in place, and a graded board of real servers. `uvx mcp-migrate check .`
